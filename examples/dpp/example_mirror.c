...
static int example_mirror_setup(int unit, int mode, int source_port,
    int probe_port);
...
int main(int argc, char *argv[])
{
  ...
  rv = example_mirror_setup(unit, mode, source_port, probe_port);
  if(rv != OPENNSL_E_NONE) {
 ...
 * \param    source_port   [IN] Source Port
 * \param    probe_port    [IN] Destination/Probe Port
 *
 * \return      OPENNSL_E_xxx  OpenNSL API return code
 *****************************************************************************/
static int example_mirror_setup(int unit, int mode, int source_port,
...
{
  int rv;
  opennsl_gport_t gp_source_port;
  ...
  OPENNSL_GPORT_LOCAL_SET(mirror_dest.gport, probe_port);
  OPENNSL_GPORT_LOCAL_SET(gp_source_port, source_port);
  ...
  rv = opennsl_mirror_port_dest_add(unit, gp_source_port, flags, mirror_dest.mirror_dest_id);
  if(rv != OPENNSL_E_NONE) {
